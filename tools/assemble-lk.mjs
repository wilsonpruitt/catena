...
const dir = "/Users/wilsonpruitt/catena/data/resweep-lk";
...
const out = {
  slug: "luke",
  name: "Luke",
  subtitle: "A Fulfillment Made of Scripture",
  translation: "World English Bible",
  howToRead:
    "Luke reads Israel’s scriptures as a promise now being kept. He opens not with a citation but with four songs that are themselves mosaics of the Old Testament — Mary’s Magnificat after Hannah, Zechariah’s Benedictus, the angels’ Gloria, Simeon’s Nunc Dimittis — and he sets the program in Jesus’ first sermon, unrolling Isaiah at Nazareth and announcing, “Today this Scripture has been fulfilled in your hearing.” The long road to Jerusalem retraces Israel’s prophets; the parables of mercy answer the law and the prophets on the poor, the stranger, and the lost; the Passion is narrated, as in Mark, in the suffering psalms. And it closes where it began, on the risen Christ opening “Moses and all the prophets” to show that everything written of him must be fulfilled. The chips beside each passage name the Scripture underneath it — solid for a formal quotation, dashed for an allusion, dotted for a fainter echo, ◇ for a figural pattern that borrows no words. A lighter chip and a trailing ? mark an echo that is faint or contested. Where Luke follows the Greek of the Septuagint against the Hebrew, both readings are shown.",
  pericopes: [],
};
...
for (let ch = 1; ch <= 24; ch++) {
  ...
  for (const p of arr) {
    const text = resolve("Luke " + p.ref) || p.text || "";
    if (!resolve("Luke " + p.ref)) noText.push(p.ref);
    const echoes = [];
...
writeFileSync("/Users/wilsonpruitt/catena/data/luke.json", JSON.stringify(out, null, 2) + "\n");
