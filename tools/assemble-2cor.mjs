...
const dir = "/Users/wilsonpruitt/catena/data/resweep-2cor";
...
const out = {
  slug: "2-corinthians",
  name: "2 Corinthians",
  subtitle: "A Ministry Made of Scripture",
  translation: "World English Bible",
  howToRead:
    "Paul’s most personal letter is a defense of his ministry, and he defends it from Scripture. Its center is a reading of the Exodus: Moses veiled his face when the glory faded, but the ministers of the new covenant are unveiled, transformed from glory to glory — the old covenant written on stone against the new written by the Spirit on hearts of flesh. Around that argument the letter gathers the language of the prophets and psalms: the day of salvation now come, the cheerful giver, the treasure carried in clay jars, reconciliation, the new creation in which the old has passed away, and a thorn answered by grace made perfect in weakness. The chips beside each passage name the Scripture underneath it — solid for a formal quotation, dashed for an allusion, dotted for a fainter echo, ◇ for a figural pattern that borrows no words. A lighter chip and a trailing ? mark an echo that is faint or contested. Where Paul follows the Greek of the Septuagint against the Hebrew, both readings are shown.",
  pericopes: [],
};
...
for (let ch = 1; ch <= 13; ch++) {
  ...
  for (const p of arr) {
    const text = resolve("2 Corinthians " + p.ref) || p.text || "";
    if (!resolve("2 Corinthians " + p.ref)) noText.push(p.ref);
    const echoes = [];
...
writeFileSync("/Users/wilsonpruitt/catena/data/2-corinthians.json", JSON.stringify(out, null, 2) + "\n");
