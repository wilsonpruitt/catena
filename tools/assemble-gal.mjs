...
const dir = "/Users/wilsonpruitt/catena/data/resweep-gal";
...
const out = {
  slug: "galatians",
  name: "Galatians",
  subtitle: "A Freedom Made of Scripture",
  translation: "World English Bible",
  howToRead:
    "To churches tempted back under the law, Paul argues for freedom — and he argues it from the Law itself. The hinge is Abraham, who believed God and was counted righteous before he was ever circumcised, and to whom the promise came that in him all the nations would be blessed. Against that promise Paul sets the law’s own curse — “cursed is everyone who hangs on a tree” — and the prophet’s word that the righteous shall live by faith. Then he reads the household of Abraham as an allegory: Hagar the slave and Sarah the free, two covenants, two Jerusalems, and the command to cast out the slave woman so that the children of promise inherit. The letter ends where it began, in freedom — not the flesh but the Spirit, not the old order but a new creation. The chips beside each passage name the Scripture underneath it — solid for a formal quotation, dashed for an allusion, dotted for a fainter echo, ◇ for a figural pattern that borrows no words. A lighter chip and a trailing ? mark an echo that is faint or contested. Where Paul follows the Greek of the Septuagint against the Hebrew, both readings are shown.",
  pericopes: [],
};
...
for (let ch = 1; ch <= 6; ch++) {
  ...
  for (const p of arr) {
    const text = resolve("Galatians " + p.ref) || p.text || "";
    if (!resolve("Galatians " + p.ref)) noText.push(p.ref);
    const echoes = [];
...
writeFileSync("/Users/wilsonpruitt/catena/data/galatians.json", JSON.stringify(out, null, 2) + "\n");
