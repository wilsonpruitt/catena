...
const dir = "/Users/wilsonpruitt/catena/data/resweep-1cor";
...
const out = {
  slug: "1-corinthians",
  name: "1 Corinthians",
  subtitle: "A Wisdom Made of Scripture",
  translation: "World English Bible",
  howToRead:
    "Writing to a fractious church enamored of cleverness, Paul argues that the wisdom of God is the crucified Christ — and he argues it out of Scripture. The opening chapters drive home the point with a chain of citations: “I will destroy the wisdom of the wise,” “let him who boasts boast in the Lord,” “no eye has seen what God has prepared,” “he catches the wise in their craftiness.” Then the letter turns the Exodus into a warning written for the church — the fathers under the cloud, baptized into Moses, drinking from the rock that was Christ, falling in the wilderness — and ends on resurrection read straight from the prophets: death swallowed up in victory, its sting drawn, the last Adam undoing the first. The chips beside each passage name the Scripture underneath it — solid for a formal quotation, dashed for an allusion, dotted for a fainter echo, ◇ for a figural pattern that borrows no words. A lighter chip and a trailing ? mark an echo that is faint or contested. Where Paul follows the Greek of the Septuagint against the Hebrew, both readings are shown.",
  pericopes: [],
};
...
for (let ch = 1; ch <= 16; ch++) {
  ...
  for (const p of arr) {
    const text = resolve("1 Corinthians " + p.ref) || p.text || "";
    if (!resolve("1 Corinthians " + p.ref)) noText.push(p.ref);
    const echoes = [];
...
writeFileSync("/Users/wilsonpruitt/catena/data/1-corinthians.json", JSON.stringify(out, null, 2) + "\n");
