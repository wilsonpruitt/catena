...
const dir = "/Users/wilsonpruitt/catena/data/resweep-rom";
...
for (let ch = 1; ch <= 16; ch++) {
  ...
  for (const p of arr) {
    const text = resolve("Romans " + p.ref) || p.text || "";
    if (!resolve("Romans " + p.ref)) noText.push(p.ref);
    const echoes = [];
...
writeFileSync("/Users/wilsonpruitt/catena/data/romans.json", JSON.stringify(out, null, 2) + "\n");
