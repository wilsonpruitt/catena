...
const dir = "/Users/wilsonpruitt/catena/data/resweep-mt";
...
const out = {
  slug: "matthew",
  name: "Matthew",
  subtitle: "A Gospel Made of Scripture",
  translation: "World English Bible",
  howToRead:
    "No book in the New Testament cites the Old more openly than Matthew. Ten times he stops the narrative to say a thing happened “that it might be fulfilled which was spoken by the prophet,” and the formula quotations — Isaiah, Hosea, Micah, Jeremiah, Zechariah, the Psalms — are only the surface. Beneath them runs a deeper weave: Jesus relives Israel’s exodus and exile, gives Torah from a new mountain, and dies inside Psalm 22. The chips beside each passage name the Scripture underneath it — solid for a formal quotation, dashed for an allusion, dotted for a fainter echo, ◇ for a figural pattern that borrows no words. A lighter chip and a trailing ? mark an echo that is faint or contested. Where Matthew follows the Greek of the Septuagint against the Hebrew, both readings are shown.",
  pericopes: [],
};
...
for (let ch = 1; ch <= 28; ch++) {
  ...
  for (const p of arr) {
    const text = resolve("Matthew " + p.ref) || p.text || "";
    if (!resolve("Matthew " + p.ref)) noText.push(p.ref);
    const echoes = [];
...
writeFileSync("/Users/wilsonpruitt/catena/data/matthew.json", JSON.stringify(out, null, 2) + "\n");
