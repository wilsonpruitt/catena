...
const dir = "/Users/wilsonpruitt/catena/data/resweep-phil";
...
const out = {
  slug: "philippians",
  name: "Philippians",
  subtitle: "A Joy Made of Scripture",
  translation: "World English Bible",
  howToRead:
    "Paul writes from prison, and writes of joy — but the joy rests on Scripture. At the letter’s heart is a hymn that reads the crucified and exalted Christ through Isaiah’s suffering and vindicated Servant: he emptied himself, took the form of a slave, was obedient unto death, and was therefore given the name above every name, so that — in the words Isaiah gave to God alone — “every knee shall bow and every tongue confess.” Around it Paul gathers the language of the righteous sufferer who will not be put to shame, the wilderness people who must not grumble, the children of God shining like stars in a crooked generation, and the true circumcision who worship by the Spirit. The chips beside each passage name the Scripture underneath it — solid for a formal quotation, dashed for an allusion, dotted for a fainter echo, ◇ for a figural pattern that borrows no words. A lighter chip and a trailing ? mark an echo that is faint or contested. Where Paul follows the Greek of the Septuagint against the Hebrew, both readings are shown.",
  pericopes: [],
};
...
for (let ch = 1; ch <= 4; ch++) {
  ...
  for (const p of arr) {
    const text = resolve("Philippians " + p.ref) || p.text || "";
    if (!resolve("Philippians " + p.ref)) noText.push(p.ref);
    const echoes = [];
...
writeFileSync("/Users/wilsonpruitt/catena/data/philippians.json", JSON.stringify(out, null, 2) + "\n");
